...
createVisitedObject(){
    let arr = {}; for (let key of this.AdjList.keys()) {
        arr[key] = false;
    }
...
bfs(startingNode){
    console.log('\nBFS');  let visited = this.createVisitedObject(); let q = []; visited[startingNode] = true; q.push(startingNode);
...
dfs(startingNode){
    console.log('\nDFS');
     let visited = this.createVisitedObject(); this.dfsHelper(startingNode, visited);
}
...
doesPathExist(firstNode, secondNode){
    let path = []; let visited = this.createVisitedObject(); let q = []; visited[firstNode] = true;
    q.push(firstNode); while (q.length) {
        let node = q.pop(); 
        path.push(node); 
