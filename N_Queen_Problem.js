...
var print_board = function (columns) {
    var n = columns.length, row = 0, col = 0; while (row < n) {
        while (col < n) {
            console.log(columns[row] === col ? 'Q ' : '# '); col++;
        }
...
var has_conflict = function (columns) {
    var len = columns.length, last = columns[len - 1], previous = len - 2; while (previous >= 0) {
        ...
        if (last + (len - 1) === columns[previous] + previous) return true; previous--;
    }
...
var place_next_queen = function (total, queens, columns) {
    ...
    for (var column = 0; column < total; column++) {
        columns.push(column); iterations++;
...
print_board(place_next_queen(17, 12));
